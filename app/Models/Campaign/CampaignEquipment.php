...
class CampaignEquipment extends Model
{
    ...
    /**
     * Only equipment that contributes to Campaign Rating (pg 19). Excludes
     * instances acquired via routes that "never count towards your campaign
     * rating" (Lucky Upstart / leader-build Special free equipment, Loot Their
     * Stash). Combine with active() for the CR tally.
     */
    public function scopeCountsTowardCr(Builder $query): Builder
    {
        return $query->where('excludes_from_cr', false);
    }
}
