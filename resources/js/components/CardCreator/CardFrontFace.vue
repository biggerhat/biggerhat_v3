...
<template>
    <div
        class="card-face card-front relative flex h-full w-full flex-col overflow-hidden rounded-lg bg-neutral-900 text-white"
        :style="{ '--faction-color': `var(${factionVar})` }"
    >
        <!-- Faction border top -->
        ...
        <div class="grid grid-cols-6 divide-x divide-white/10 text-center" :style="{ background: `hsl(var(${factionVar}) / 0.2)` }">
            ...
            <div class="py-1.5">
                ...
                <div class="flex flex-wrap items-center justify-center gap-0.5 px-1">
                    ...
                    <GameIcon type="soulstone" class-name="ml-0.5 text-xs shrink-0" />
                </div>
            </div>
        </div>
        ...
        <!-- Abilities -->
        <div v-if="abilities.length" class="px-2.5 py-2 text-base leading-6">
            <div
