...
<template>
    <div class="container flex flex-1 flex-col gap-4 rounded-xl p-4 mb-8 mx-auto">
        <div class="grid auto-rows-min gap-2 md:grid-cols-8">
            <div class="flex flex-col space-y-1.5 md:col-span-4"
                 v-if="props.miniature.combination_image && !isMobileDevice()">
                <img :src='"/storage/" + props.miniature.combination_image' :alt="miniature.display_name"
                     class="rounded">
            </div>
                ...
                <Card class="w-full rounded-none border-none m-0 p-0">
                    ...
                    <CardContent class="px-0 border-l border-r py-0">
                        <Link :href="route('factions.view', character.faction)"
                              class="block p-2 m-0 w-full h-full text-md border-b hover:bg-secondary"
                        ...
                        <div class="border-primary" v-if="character.keywords.length > 0">
                            <Link :href="route('keywords.view', keyword.slug)"
                                  class="block p-2 m-0 w-full h-full border-b hover:bg-secondary text-md"
                        ...
                        <div class="border-primary" v-if="character.crew_upgrades.length > 0">
                            <Link :href="route('upgrades.view', upgrade.slug)"
                                  class="block p-2 m-0 w-full h-full border-b hover:bg-secondary text-md"
                        ...
                        <div class="border-primary" v-if="character.totem">
                            <Link
                                :href="route('characters.view', {character: character.totem.slug, miniature: character.totem.standard_miniatures[0].id, slug: character.totem.standard_miniatures[0].slug})"
                                class="block p-2 m-0 w-full h-full border-b hover:bg-secondary text-md">
                        ...
                        <div class="border-primary" v-if="character.is_totem_for">
                            <Link
                                :href="route('characters.view', {character: character.is_totem_for.slug, miniature: character.is_totem_for.standard_miniatures[0].id, slug: character.is_totem_for.standard_miniatures[0].slug})"
                                class="block p-2 m-0 w-full h-full border-b hover:bg-secondary text-md">
