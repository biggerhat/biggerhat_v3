...
const columns: ColumnDef<Asset>[] = [
    ...
    {
        accessorKey: 'scrip_cost',
        header: () => h('div', {}, 'Scrip'),
        cell: ({ row }) => h('div', { class: 'tabular-nums' }, row.getValue('scrip_cost')),
    },
    {
        id: 'allegiances',
        accessorFn: (r) => r.allegiances.map((a) => a.name).join(', '),
    ...
    },
    {
        id: 'limits',
        header: () => h('div', {}, 'Limits'),
        cell: ({ row }) => h('div', { class: 'text-[11px] capitalize' }, row.original.limits.map((l) => l.limit_type).join(', ')),
    },
    {
        id: 'actions',
        enableHiding: false,
...
<template>
    ...
    <PageBanner title="TOS Assets" class="mb-2">
        <template #actions>
            <Button class="my-auto mr-2" @click="router.get(route('admin.tos.assets.create'))">Create Asset</Button>
        </template>
    </PageBanner>
