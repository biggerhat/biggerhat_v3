...
// job finishes — updated_at is the only reliable "the new image is ready"
// signal. Without this, taking/undoing an advancement while staying on this
// page showed a stale image until the user navigated away and back.
...
const pollCardRegeneration = (target: 'leader' | 'totem', previousUpdatedAt: string | undefined, attemptsLeft = 8) => {
    cardRegenerating.value[target] = true;
    setTimeout(() => {
        router.reload({
            only: ['leader', 'totem'],
            onSuccess: () => {
                const current = (target === 'leader' ? props.leader : props.totem)?.updated_at;
                if (current && current !== previousUpdatedAt) {
                    cardRegenerating.value[target] = false;
                    cardCacheBust.value[target] = Date.now();
                } else if (attemptsLeft > 1) {
                    pollCardRegeneration(target, previousUpdatedAt, attemptsLeft - 1);
                } else {
                    cardRegenerating.value[target] = false;
                }
            },
...
const logAdvancement = (position: number) => {
    ...
    const prevLeaderUpdatedAt = props.leader?.updated_at;
    const prevTotemUpdatedAt = props.totem?.updated_at;
    router.post(
        ...
        {
            onSuccess: () => {
                if (targetsLeader) pollCardRegeneration('leader', prevLeaderUpdatedAt);
                if (targetsTotem) pollCardRegeneration('totem', prevTotemUpdatedAt);
            },
        },
    );
};
...
const removeAdvancement = async (a: AdvancementTaken) => {
    ...
    // it just won't see updated_at change and will stop after its attempts.
    const prevLeaderUpdatedAt = props.leader?.updated_at;
    const prevTotemUpdatedAt = props.totem?.updated_at;
    router.delete(route('campaigns.crews.leader.advancements.destroy', [props.campaign.id, props.crew.share_code, a.id]), {
        onSuccess: () => {
            pollCardRegeneration('leader', prevLeaderUpdatedAt);
            if (props.totem) pollCardRegeneration('totem', prevTotemUpdatedAt);
        },
    });
};
...
const searchAddUnit = (q: string) => {
    ...
    addUnitDebounce = setTimeout(async () => {
        try {
            ...
            addUnitResults.value = await res.json();
        } catch {
            addUnitResults.value = [];
...
const submitAddUnit = () => {
    ...
    router.post(
        ...
        { character_id: addUnitSelected.value.id, label: addUnitLabel.value || null },
            ...
            <DialogContent class="max-w-sm">
                ...
                <template v-else>
                    <div class="flex items-center gap-2 rounded-md border p-2">
                        ...
                        />
                        <div class="min-w-0 flex-1">
                            <div class="truncate font-medium">{{ addUnitSelected.display_name }}</div>
