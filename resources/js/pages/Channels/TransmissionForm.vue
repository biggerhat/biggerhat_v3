...
const submit = () => {
    if (props.transmission) {
        router.post(route('transmissions.update', { channel: props.channel.slug, transmission: props.transmission.slug }), formInfo.value);
    } else {
        router.post(route('transmissions.store', { channel: props.channel.slug }), formInfo.value);
    }
};
...
<template>
    <Head :title="`${transmission ? 'Edit' : 'New'} Transmission - ${channel.name}`" />

    <div class="container mx-auto mt-6">
        <Card>
            <CardHeader>
                <CardTitle>{{ transmission ? 'Edit' : 'New' }} Transmission</CardTitle>
                ...
                <form>
                    <div class="grid w-full items-center gap-4">
                        ...
                        <div class="flex flex-col space-y-1.5">
                            ...
                            <Textarea id="description" v-model="formInfo.description" placeholder="What is this content about?" />
                        </div>
            ...
            <CardFooter class="flex justify-between px-6 pb-6">
                <Button @click="router.get(route('channels.view', channel.slug))" variant="outline"> Cancel </Button>
